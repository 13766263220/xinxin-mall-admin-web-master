<template>
	<div>
		...
		<div>
			...
			>
				...
				<el-table-column
				  ...
				  width="160">
				  <template slot-scope="scope">
					<el-button @click="" size="mini" >编辑</el-button>
		...
		>	
			...
			<span slot="footer" class="dialog-footer" style="">
				<el-button @click="stockDialogParam.show = false" size="mini">取 消</el-button>
				<el-button @click="editStock()" type="primary" size="mini">编辑</el-button>
			</span>
		</el-dialog>
		<!-- 商品SKU库存 对话框 end -->
	...
	export default{
		...
		methods:{
			...
			changeAttrOptions(attrIndex){//选择发生的事件
				...
				//var result = '{'+ '"'+attrs[attrIndex].name +'"' + ':'+'"'+this.stockAddDialogParam.radio[attrIndex]+'"'+'}'
				//alert(result)
				//alert(JSON.parse(result))
				
				this.stockAddDialogParam.productSpecs[attrs[attrIndex].name] = this.stockAddDialogParam.radio[attrIndex];
				//alert(JSON.stringify(this.stockAddDialogParam.productSpecs))
				//console.log(this.stockAddDialogParam.productSpecs)
				
				
				//alert(attrName)
				//alert(this.stockAddDialogParam.radio[1])
			},
			insertStock(){//确认添加发生
					...
					if(valid){
						...
						if(Object.getOwnPropertyNames(this.stockAddDialogParam.productSpecs).length == this.stockAddDialogParam.productAttr.length+1){
							...
							var data={
								...
								productSpecs:JSON.stringify(this.stockAddDialogParam.productSpecs) //商品规格
							}
