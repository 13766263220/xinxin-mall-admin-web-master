<template>
	<div>
		...
		<el-button style="margin-left: 200px;" @click="next()">确认提交</el-button>
	</div>
</template>
...
<script>
	...
	export default{
		mounted() {
			//el-container scroll is-vertical
			this.queryBrandAll();//查询所有品牌
			if(this.$route.params.edit){//表示是编辑
				this.title = 'edit';
				var product = this.$route.params.edit;
				console.log(product)
				this.productAddForm.id = product.id;
				this.productAddForm.name = product.name;
				this.productAddForm.price = product.price;
				this.productAddForm.stock = product.stock;
				this.productAddForm.lowStock = product.lowStock;
				this.productAddForm.unit = product.unit;
				this.productAddForm.categoryInfo.value = [product.productCategory.parentId,product.categoryId]
				this.productAddForm.brandInfo.value = product.brandId;
				this.productAddForm.publishStatus = product.publishStatus;
				this.productAddForm.newStatus = product.newStatus;
				this.productAddForm.recommandStatus = product.recommandStatus;
				
				//赋值图片
				if (product.pic !== null && product.pic !== undefined && product.pic !== ''){
					this.productAddForm.pic.iconImagelist=[{url:product.pic}];//赋值上传图片框
					this.productAddForm.pic.iconPath=product.pic;//赋值图片存进数据库的path
				}
				
				this.productAddForm.description = product.description;
		...
		methods:{
			...
			next(){//下一步
				this.$refs["productAddForm"].validate(valid => {//判断校验是否成功
					if(valid){
						this.$emit('active',1)
						this.$emit('productForm',[this.productAddForm,this.title])
					}
				})
			},
			/* 上传 start */
